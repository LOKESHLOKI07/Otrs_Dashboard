...
footer.text_frame.paragraphs[0].font.size = Inches(0.3)

# Create a table shape on the new slide
rows = len(catergory_result) + 1  # Include header row
cols = len(catergory_result[0]) + 1

left, top, width, height = Inches(0), Inches(1.5), Inches(10), Inches(0.5)
table = slide.shapes.add_table(rows, cols, left, top, width, height).table
...
tp_query2 = '''SELECT  s.name as 'SLA',
SUM(CASE WHEN ts.id IN (1, 4, 6, 7, 8, 11, 13, 15, 16, 14, 17, 18, 2, 3, 10, 12) THEN 1 ELSE 0 END) AS 'TOTAL',
SUM(CASE WHEN ts.id IN (2, 3, 10, 12) THEN 1 ELSE 0 END) AS 'CLOSED',
SUM(CASE WHEN ts.id IN (1, 4, 6, 7, 8, 11, 13, 15, 16, 14, 17, 18) THEN 1 ELSE 0 END) AS 'Work In Process'
FROM ticket t
...
for i in range(rows):
    for j in range(cols):
        table1.cell(i + 1, j).text = str(df1.values[i, j])
...
logo = slide.shapes.add_picture(logo_path, Inches(2), Inches(3.5), Inches(5), Inches(3))

# add a header to the slide
header = slide.shapes.add_textbox(Inches(0.5), Inches(2.5), Inches(9), Inches(0.5))
header.text = 'Thank You'
header.text_frame.paragraphs[0].font.size = Inches(0.4)
# set font size and color
# header.text_frame.paragraphs[0].font.size = Inches(0.4)
# header.text_frame.paragraphs[0].font.color.rgb = RGBColor(255, 0, 0)  # set color to red

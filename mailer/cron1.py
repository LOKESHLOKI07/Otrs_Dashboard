...
def schedule_project():
    ...
    for data in all_val:
        ...
        subprocess.run(
            ['python3', f"/home/ubuntu/reportproject/rameez/{engineer_name}.py", *to_list, *cc_list])

        # Log the result of the script execution
        # logging.info(f"Result for {engineer_name}:")
...
if __name__ == "__main__":
    ...
    # val = Student.objects.filter(hour=hours, minutes=minutes, days__contains=days).values()
    # print(val)
    # instances = Student.objects.filter(hour=hours, minutes=minutes, days__contains=days)
    # print(instances)

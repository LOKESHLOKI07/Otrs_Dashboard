...
type_query = """
 SELECT  tt.name,
  SUM(CASE WHEN ts.id IN (1,4,6,7,8,11,13, 15, 16, 14, 17,18) and t.create_time BETWEEN DATE_SUB(NOW(), INTERVAL 7 day) AND NOW() THEN 1 ELSE 0 END) AS pending,
SUM(CASE WHEN ts.id IN (2, 3,10,12) and t.create_time BETWEEN DATE_SUB(NOW(), INTERVAL 7 day) AND NOW() THEN 1 ELSE 0 END) AS closed,
SUM(CASE WHEN ts.id IN (2, 3,10,12) and t.change_time BETWEEN DATE_SUB(NOW(), INTERVAL 7 day) AND NOW() THEN 1 ELSE 0 END) AS 'overallClosed'
FROM 
  ticket AS t 
...
one_week_ago = (today - timedelta(weeks=1)).strftime("%Y-%m-%d")
html_table += f"<p style='font-size: 20px;'> Dear team, <br> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Please find the below statistics of Weekly Registered Ticket Summary from {one_week_ago} 06:00 AM to {today} 06:00 AM </p>"
...
one_week_ago = (date - timedelta(weeks=1)).strftime("%Y-%m-%d")
...
with smtplib.SMTP('webmail.futurenet.in', 587) as mail:
    ...
    message['subject'] = "Weekly Registered Ticket Summary" + one_week_ago + ' 6:00 AM to ' + today + ' 6:00 AM '
